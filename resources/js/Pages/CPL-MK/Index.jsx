...
export default function Index(props) {
    const { data: mks, meta, links } = props.mks;
                ...
                <Card>
                    ...
                    <CardContent className="p-0 [&-td]:whitespace-nowrap [&-td]:px-6 [&-th]:px-6">
                        {mks.length === 0 ? (
                            <EmptyState
                                icon={IconArrowsSplit}
                                title="Tidak ada pemetaan CPL-MK"
                                subtitle="Mulailah dengan membuat pemetaan CPL-MK baru"
                            />
                        ) : (
                            <Table className="w-full">
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead>ID MK</TableHead>
                                        <TableHead>CPL Terhubung</TableHead> {/* <-- 1. TAMBAH HEADER BARU */}
                                ...
                                <TableBody>
                                    {mks.map((mk, index) => (
                                        <TableRow key={mk.id}>
                                            ...
                                            {isSuperAdmin && <TableCell>{mk.prodi?.name}</TableCell>}
                                            <TableCell>
                                                <Tooltip>
                                            ...
                                            <TableCell>
                                                ...
                                                {mk.programLearningOutcomes && mk.programLearningOutcomes.length > 0 ? (
                                                    <div className="flex flex-wrap gap-1">
                                                        {/* Lakukan perulangan dan tampilkan sebagai Badge */}
                                                        {mk.programLearningOutcomes.map((cpl) => (
                                                            <Tooltip key={cpl.id}>
                                                                <TooltipTrigger>
                                                                    <Badge variant="secondary">{cpl.code}</Badge>
                                                                </TooltipTrigger>
                                                                <TooltipContent>
                                                                    <p>{cpl.description}</p>
                                                                </TooltipContent>
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-x-1">
                                                    <Button variant="blue" size="sm" asChild>
                                                        <Link href={route('cpl-mk.edit', [mk])}>
                                                    ...
                                                    <AlertAction
                                                        ...
                                                        action={() => deleteAction(route('cpl-mk.destroy', [mk]))}
                                                    />
                                                </div>
                                            </TableCell>
                                        </TableRow>
