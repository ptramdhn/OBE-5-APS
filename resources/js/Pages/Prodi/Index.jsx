...
import { IconArrowsDownUp, IconPencil, IconPlus, IconSchool, IconTrash } from '@tabler/icons-react';
import { useEffect, useState } from 'react';
import { toast } from 'sonner';

export default function Index(props) {
    const { data: programStudies, meta, links } = props.programStudies;
    ...
    UseFilter({
        route: route('program-studies.index'),
        values: params,
        only: ['programStudies'],
    });
                            ...
                            <TableBody>
                                {programStudies.map((programStudy, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>{programStudy.code}</TableCell>
                                        <TableCell>{programStudy.name}</TableCell>
                                        <TableCell>{formatDateIndo(programStudy.created_at)}</TableCell>
                                        <TableCell>
                                            <div className="flex items-center gap-x-1">
                                                <Button variant="blue" size="sm" asChild>
                                                    <Link href={route('program-studies.edit', [programStudy])}>
                                                ...
                                                <AlertAction
                                                    ...
                                                    action={() =>
                                                        deleteAction(route('program-studies.destroy', [programStudy]))
                                                    }
                                                />
                                            </div>
                                        </TableCell>
