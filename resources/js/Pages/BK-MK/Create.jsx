...
export default function Create(props) {
    // Sesuaikan nama props: sekarang kita menerima `bks` dan `mks`
    const { bks, mks } = props;

    // State untuk menampung daftar MK yang sudah difilter
    const [filteredMks, setFilteredMks] = useState(mks);

    const { data, setData, errors, post, processing, reset } = useForm({
        bk_id: '', // Field utama sekarang adalah `bk_id`
        mk_ids: [{ id: Date.now(), value: '' }], // Array dinamis untuk MK
    });

    // useEffect untuk memfilter daftar MK berdasarkan prodi dari BK yang dipilih
    useEffect(() => {
        if (!data.bk_id) {
            setFilteredMks(mks);
            return;
        ...
        // Reset pilihan MK yang sudah ada
